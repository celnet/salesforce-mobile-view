...
var AjaxPools = (function(){
        ...
        Ajax.ajax(
            ...
            function(response){
                if(response.results != null && response.results.length > 0){
					welinkStorage['welink_' + sobjectName + '_describeLayoutsAndSearchFields'] = JSON.stringify(response);
					AjaxResponses.describeLayoutsAndSearchFields = response;
                    AjaxProcessor.handleDescribeLayoutsAndSearchFields(response);
                }
    ...
    var retrieveLayouts = function(sobjectName, callbackFunction){
        var requestPrepares = AjaxProcessor.prepareLayoutDetailRequestBody(AjaxResponses.layouts.recordTypeMappings);
        var reqBody = requestPrepares.reqBody;
        var recordTypeIds = requestPrepares.recordTypeIds;
        
        if(reqBody.batchRequests.length == 0){
        ...
        Ajax.ajax(
            ...
            reqBody, 
            function(response){
				welinkStorage['welink_' + sobjectName + '_layoutDetails'] = JSON.stringify(response);
				AjaxResponses.layoutDetails = response;
				
                AjaxProcessor.handleLayoutDetails(response, recordTypeIds);
